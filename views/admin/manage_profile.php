...
/* ===== Helper: xác định user bị ban qua fullname prefix ===== */
function isBannedUserRow(array $row): bool {
    if (!isset($row['fullname']) || $row['fullname'] === null) return false;
    return str_starts_with($row['fullname'], '[BANNED]');
}
        ...
        if (!$userRow) {
            ...
            if ($action === 'delete') {
                ...
                if (isBannedUserRow($userRow)) {
                    ...
                    // Ban: đổi password random + thêm prefix [BANNED] vào fullname
                    $randomPassword = bin2hex(random_bytes(16));
                    $hash           = password_hash($randomPassword, PASSWORD_BCRYPT);
                    $newName        = '[BANNED] ' . ($userRow['fullname'] ?? $userRow['username']);

                    $stmt = $pdo->prepare("UPDATE users SET password = ?, fullname = ? WHERE id = ?");
                ...
                if (!isBannedUserRow($userRow)) {
                    ...
                    $cleanName = stripBannedPrefix($userRow['fullname']);
    ...
    if ($editId <= 0 || $username === '') {
        ...
        if ($stmt->fetch()) {
            ...
            if ($new_pass !== '') {
                ...
                $stmt   = $pdo->prepare("UPDATE users SET username = ?, fullname = ?, is_admin = ?, password = ? WHERE id = ?");
                $params = [$username, $fullname === '' ? null : $fullname, $is_admin, $hash, $editId];
            } else {
                $stmt   = $pdo->prepare("UPDATE users SET username = ?, fullname = ?, is_admin = ? WHERE id = ?");
                $params = [$username, $fullname === '' ? null : $fullname, $is_admin, $editId];
            }
                  ...
                  <div class="modal-body">
                    ...
                    <div class="mb-3">
                      ...
                      <input
                        ...
                        class="form-control"
                        name="fullname"
                        value="<?= htmlspecialchars(stripBannedPrefix($u['fullname'] ?? '')) ?>"
